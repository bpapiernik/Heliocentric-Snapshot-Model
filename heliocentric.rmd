...
final_shots <- shots3 %>%
  select(court_zone, shot_distance, defender_distance, points, shot_made_flag)
...
```{r}
dtrain <- xgb.DMatrix(data = as.matrix(shot_train[,c(1:3)]), label = as.numeric(shot_train$points))

# Create test matrix

dtest <-  xgb.DMatrix(data = as.matrix(shot_test[,c(1:3)]), label = as.numeric(shot_test$points))
```
...
```{r}
xgb.save(bst_final, "heliocentric_xg.model")
```
